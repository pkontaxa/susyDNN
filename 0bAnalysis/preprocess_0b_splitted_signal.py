...
work_dir = '/afs/cern.ch/work/p/pakontax/private/susyDNN/susyDNN/'
out_dir = work_dir+'/FOR_COPY/preprocessedData_Adversarial_3August2020_0b/'
skimmed_dir = work_dir+'FOR_COPY/preprocessedData_Adversarial_3August2020_0b/skimmed_nJets_geq3_dPhi05_nBJet0/'
...
bkg_filelist = work_dir+'background_0b.txt'
...
# Remove nWMedium or nWTight
df_bkg = df_bkg.drop(columns=['nWTight'])
df_sig = df_sig.drop(columns=['nWTight'])

#df_bkg = df_bkg.drop(columns=['nWMedium'])
#df_sig = df_sig.drop(columns=['nWMedium'])
...
discrete_cols = ['nWMedium', 'nJets30Clean']
...
hickle.dump(list(train), out_dir+'All_input_features.hkl')
hickle.dump(norm_cols, out_dir+'Normalized_input_features.hkl')
...
joblib.dump(minmaxScaler, out_dir+'minmaxScaler.pkl')
...
train_out=out_dir+'train_set.root'
train.to_root(train_out, key='tree')

test_out=out_dir+'test_set.root'

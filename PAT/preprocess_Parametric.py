...
out_dir = work_dir+'/preprocessedData_Parametric_ONLY_tt2lep/'
skimmed_dir = work_dir+'preprocessedData_Parametric_ONLY_tt2lep/skimmed_nJets_geq4_dPhi05/'

signal_name = ['total_SIGNAL']
for mass_point in signal_name:
    ### LOAD SIGNAL FILE ###
    signal_file = skimmed_dir+'evVarFriend_'+str(mass_point)+'_skimmed.root'
    ...
    bkg_filelist = work_dir+'background_file_list_Pantelis_nJets_geq4_dPhi05_PARAMETRIC_Only_tt2lep.txt'
    ...
    # Shuffle initial signal sample
    df_sig = df_sig.sample(frac=1, random_state=42)
    ...
    discrete_cols = ['nTop_Total_Combined', 'nJets30Clean', 'nResolvedTop', 'nBCleaned_TOTAL', 'dM_Go_LSP']
    ...
    #Pantelis normScaler = StandardScaler().fit( train[norm_cols].values )
    ...
    print 'Parametric ', mass_point, ' processed!'
